    ...
    public partial class Form1 : Form
    {
        ...
        /* private byte[]? lastReceivedA = null; // 存储上次收到的 A
         ...
         private bool TryMergeAndCheckDefects(byte[]? dataA, byte[]? dataB)
         {
             ...
             int defectCountA = BitConverter.ToInt32(dataA, 1);
             int defectCountB = BitConverter.ToInt32(dataB, 1);
             ...
             for (int i = 5; i < dataA.Length; i++)
             {
                 bool detectedA = (dataA[i] == 0xA0);
                 bool detectedB = (dataB[i] == 0xA0);

                 if (!detectedA && !detectedB)
                 {
                     MessageBox.Show($"缺陷位置 {i - 5} 未检出，检测不合格！", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                     return false;
                 }
             }
